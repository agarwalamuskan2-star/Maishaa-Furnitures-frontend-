...
const GazeboDaybedCollection = () => {
    useEffect(() => {
        document.title = "Luxury Outdoor Gazebo Daybeds | Maishaa";
    }, []);

    const products: (Product & { description?: string, bestSeller?: boolean, emi?: string, discountLabel?: string })[] = [
        {
            name: "Lydia Outdoor Gazebo Daybed",
            price: "2,45,000",
            emi: "20,416",
            tag: "MADE TO ORDER",
            description: "A sanctuary for your garden. Features a robust teak frame with sun-resilient fabric drapes.",
            bestSeller: true,
    ...
    const availableTypes = [
        { label: "Gazebos", value: "Gazebos", count: products.filter(p => p.type === "Gazebos").length }
    ].filter(t => t.count > 0);
            ...
            <main className="flex-1">
                ...
                <div className="pt-6 pb-4 px-4 sm:px-8 md:px-16 lg:px-20 bg-white">
                    <nav className="flex items-center gap-2 text-gray-400 text-[11px] tracking-wide uppercase">
                        ...
                        <ChevronRight size={12} />
                        <span className="text-black font-semibold">Gazebo Daybed</span>
                    </nav>
                </div>
                        ...
                        <div className="flex-1">
                            <div className="flex flex-col md:flex-row justify-between items-start mb-12 gap-6">
                                <div className="space-y-2">
                                    <h1 className="text-2xl font-serif font-medium uppercase tracking-wider">Outdoor Gazebo Daybeds</h1>
...
export default GazeboDaybedCollection;

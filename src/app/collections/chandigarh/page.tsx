                ...
                <section className="py-20 px-4 sm:px-8 md:px-16 lg:px-20 bg-white border-t border-gray-50">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-10 gap-y-20">
                        {products.map((product, idx) => (
                            ...
                            >
                                <div className="relative aspect-square overflow-hidden mb-6 bg-gray-50 border border-transparent hover:border-gray-100 transition-all">
                                    <Image
                                        src={product.image}
                                        alt={product.name}
                                        fill
                                    ...
                                    />
                                    <button className="absolute bottom-6 right-6 p-3 bg-white rounded-full shadow-lg opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300 hover:text-orange-600 text-gray-400">
                                        <Heart size={20} strokeWidth={1.5} />
                                    </button>
                                    <div className="absolute top-6 left-6">
                                        <span className="bg-black text-white px-3 py-1 text-[9px] font-bold tracking-[0.2em] uppercase shadow-md">
                                            {product.tag}
                                        </span>
                                    </div>
                                    {product.bestSeller && (
                                        <div className="absolute top-6 right-6">
                                            <span className="bg-orange-600 text-white px-3 py-1 text-[9px] font-bold tracking-[0.2em] uppercase shadow-md">
                                                BEST SELLER
                                            </span>
                                        </div>
                                    )}
                                ...
                                <div className="space-y-3 px-2">
                                    <h3 className="text-lg font-medium text-gray-900 group-hover:text-orange-600 transition-colors uppercase tracking-tight">
                                        {product.name}
                                    </h3>
                                    <div className="flex items-baseline gap-2">
                                        <p className="text-xl font-bold text-gray-900 tabular-nums">₹{product.price}</p>
                                        <p className="text-sm text-gray-400 line-through tabular-nums">₹{product.originalPrice}</p>
                                    </div>
                                </div>
                            </motion.div>
                        ))}
